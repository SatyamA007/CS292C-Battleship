...
class Puzzle:
    ...
    def print_solution_to_file_rosette(self):
    	...
    	# Print solution given in instance generation to file
        f = open(puzzle_path_dir+'/'+puzzle_size+'_'+str(self.battleships)+'.'+str(self.cruisers)+'.'+str(self.destroyers)+'.'+str(self.submarines)+'.txt', 'w')
        
        row_totals = []
        for j in range(0, self.m):
            row_totals.append(str(self.row_totals[j]))
            row_totals.append(" ")
        f.write(''.join(row_totals) + '\n')
        ...
        #Constraints, add ship segment and water positions
        ship_constraints = []
        total_ship = (self.battleships*self.battleship_size+self.cruisers*self.cruiser_size+self.destroyers*self.destroyer_size+self.submarines*self.submarine_size)
        water_constraints = []
        ...
        f.write(''.join(water_constraints))
